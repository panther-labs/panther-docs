...
<summary>CloudFormation Template File</summary>
...
<summary>I want to set everything up on my own</summary>

Create the role yourself and fill in the role ARN in Panther.
...
Note: If you choose the option **I want to set everything up on my own** to create an IAM role, you must also complete the steps under [Set up notifications of new data](s3.md#set-up-notifications-of-new-data) to configure your S3 buckets to send notifications when new data arrives.
...
We recommend that you create a log drop-off alarm to alert you if data stops flowing from the log source. Be sure to set an appropriate time interval for when you would like Panther to alert you that the log source is not sending data.
...
Note that if you created an IAM role manually, you must also complete the steps under [Set up notifications of new data](s3.md#set-up-notifications-of-new-data) to configure your S3 buckets to send notifications when new data arrives.
...
Once the S3 bucket has been successfully onboarded to Panther and data is flowing, Panther will backup all raw logs for up to 30 to 90 days. After up to 90 days, the logs will be deleted.&#x20;
...
Follow the steps below if you chose to create an IAM role manually.&#x20;
...
1. Create an SNS topic (or modify an existing SNS topic)
2. Create the subscription to the Panther Master account's SQS queue
3. Configure event notifications on the bucket
...
3. Select **Create Stack** (with new resources).\
   ![](<../../../../.gitbook/assets/setup-sns1 (12) (1) (1) (11) (1) (1) (12).png>)\
...
4.  Under the "Specify template" section, enter the following Amazon S3 URL:\
    ...
    \
    ![](<../../../../.gitbook/assets/setup-sns2 (11) (1) (1) (11) (1) (1) (12).png>)\
